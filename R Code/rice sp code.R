...
library(data.table)
sp <- fread("rice (2F splitplot) LMER.txt")

sp$Block <- as.factor(sp$Block)
sp$N     <- as.factor(sp$N)
sp$Var   <- as.factor(sp$Var)
sp$Yield <- as.numeric(sp$Yield)
...
boxplot(data=sp, Yield ~ N      , las=2)
boxplot(data=sp, Yield ~ Var    , las=2)
